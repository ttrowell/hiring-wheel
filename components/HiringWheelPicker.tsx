...
const colors = ['#FF6B6B', '#5ECFC4', '#45B7D1', '#FFA07A', '#98D8C8', '#F7DC6F', '#BB8FCE', '#F1948A'];
const options = [
    "Best Teammate",
    "Most Compatible",
    "Hardest-worker",
...
const HiringWheelPicker = () => {
    ...
    const spinWheel = () => {
        ...
        setTimeout(() => {
            setIsSpinning(false);
            setSelectedOption(options[Math.floor(Math.random() * options.length)]);
        }, 5000);
    };

    return (
        <div className="flex flex-col items-center justify-center h-screen bg-gray-100 text-xl">
            Hiring Manager Helper
            ...
            <div className="flex flex-col items-center justify-center h-screen bg-gray-100">
                        ...
                        {options.map((option, index) => (
                            <g key={index} transform={`rotate(${index * 45} 50 50)`}>
                                ...
                                <text
                                    ...
                                    fill="black"
                                    textAnchor="end"
                                    transform="rotate(180 44 50) translate(10 45)"
                                >
                                    {option}
                                </text>
                ...
                >
                    {isSpinning ? 'The Offer Goes to...' : 'Spin the Wheel'}
                </Button>
                {selectedOption && (
